...
TArray<FVector2D> URPGridGenerator::Generate2D(const FVector2D& Origin, const FVector2D& Span, float Separation)
{
...
TArray<FVector> URPGridGenerator::Generate3D(const FVector& Origin, const FVector& Span, float Separation)
{
	if(FMath::IsNearlyZero(Separation, 0.001f))
	{
		return {};
